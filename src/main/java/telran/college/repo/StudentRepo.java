...
public interface StudentRepo extends JpaRepository<Student, Long> {
    ...
    @Query("SELECT s.name as name, AVG(m.score) as score FROM Student s JOIN s.marks m GROUP BY s.name ORDER BY AVG(m.score) DESC")
    List<NameScore> studentsMarks();

    @Query("SELECT s.name as name, s.phone as phone FROM Student s WHERE MONTH(s.birthDate) = :month")
    List<NamePhone> findStudentsBurnMonth(int month);
}
